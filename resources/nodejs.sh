#!/bin/bash
cd $1
touch /tmp/mySensors_dep
echo "Début de l'installation"

echo 0 > /tmp/mySensors_dep
DIRECTORY="/var/www"
if [ ! -d "$DIRECTORY" ]; then
...
echo 80 > /tmp/mySensors_dep
sudo npm install --unsafe-perm serialport
echo 85 > /tmp/mySensors_dep
sudo npm install --unsafe-perm request
echo 90 > /tmp/mySensors_dep

sudo chown -R www-data *

rm /tmp/mySensors_dep
